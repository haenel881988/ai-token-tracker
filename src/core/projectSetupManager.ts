...
export interface ProjectSetupConfig {
    ...
    createGithubInstructions: boolean;
    createModularizationWorkflow: boolean;
    tokenLimit: number;
    projectType: string;
}
...
export class ProjectSetupManager {
    ...
    public async setupProjectStructure(): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                '🚀 AI Token Tracker Setup erfolgreich! GitHub-Struktur wurde erstellt.',
                'Anleitung anzeigen'
            ).then(selection => {
                if (selection) {
                    this.showSetupGuide();
     ...
     */
    private async setupGithubStructure(): Promise<void> {
     ...
     */
    private async extendExistingCopilotInstructions(filePath: string): Promise<void> {
...
`;
     ...
     */
    private async createNewCopilotInstructions(filePath: string): Promise<void> {
...
`;
     ...
     */
    private async createTokenTrackerGuide(): Promise<void> {
...
`;
     ...
     */
    private async offerModularizationWorkflow(): Promise<void> {
        const choice = await vscode.window.showInformationMessage(
            '🏗️ Möchten Sie den Modularisierungs-Workflow für Ihr Projekt erstellen?',
            'Ja, Projekt analysieren',
            'Später',
            'Niemals'
        );
     ...
     */
    public async createModularizationWorkflow(): Promise<void> {
        // 1. Projekt scannen
        ...
        vscode.window.showInformationMessage(
            '📋 Modularisierungs-Workflow erstellt! Siehe docs/ai_token_tracker/scopes_ad/',
            'Workflow anzeigen'
        ).then(selection => {
            if (selection) {
     ...
     */
    private async scanDirectory(dirPath: string, structure: any): Promise<void> {
     ...
     */
    private async createScopeDirectories(): Promise<void> {
     ...
     */
    private async generateModularizationTodos(projectStructure: any): Promise<void> {
...
${projectStructure.largeSections.map((file: any, index: number) => 
    `- [ ] **01_core_modules/module_${index + 1}.md**: ${file.relativePath} modularisieren (${file.tokens} Tokens)`
).join('\n')}
...
`;
    ...
    private openModularizationWorkflow(): void {
