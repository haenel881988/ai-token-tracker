import { ITokenCounter, TokenUsage } from './interfaces';
...
export class TokenCounter implements ITokenCounter {
     ...
     */
    trackUsage(scopeId: string, tokens: number): void {
     ...
     */
    getCurrentUsage(scopeId: string): TokenUsage {
        const history = this.usageHistory.get(scopeId) || [];
     ...
     */
    calculateChatHistoryTokens(messages: string[]): number {
        let totalTokens = 0;
     ...
     */
    calculateContextualTokens(prompt: string, fileContent: string, chatHistory: string[]): number {
